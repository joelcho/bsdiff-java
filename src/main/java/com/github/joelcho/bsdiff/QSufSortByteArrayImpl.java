/**
 * This is a copy of
 * https://github.com/carrotsearch/jsuffixarrays/blob/7e9618fca76d6afb6909c96893c04798aaae1894/src/main/java/org/jsuffixarrays/QSufSort.java
 * <p/>
 * repository: https://github.com/carrotsearch/jsuffixarrays
 * <p/>
 ...
 * <p>
 * Algorithm modifies input during processing, see {@link #QSufSortByteArrayImpl(boolean)}.
 */
public class QSufSortByteArrayImpl implements QSufSort
{
     ...
     * intact.
     */
    public QSufSortByteArrayImpl(boolean preserveInput)
    {
    ...
    public IntBuffer sort(ByteBuffer buffer, int length) throws IOException {
        ...
        for (int i = 0; i < length; i++) {
            array[i] = Byte.toUnsignedInt(buffer.get(i));
        }
        int[] ints = buildSuffixArray(array, 0, length);
        IntBuffer buf = ByteBuffer.allocate((length + 1) * 4).asIntBuffer();
     ...
     * <p>
     */
    private final int [] buildSuffixArray(int [] input, int start, int length)
    {
        assertAlways(input.length >= start + length + 1, "no extra space after input end");
        MinMax minmax = minmax(input, start, length);
